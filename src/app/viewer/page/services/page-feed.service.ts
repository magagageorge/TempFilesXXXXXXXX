...
export class PageFeedService {
  ...
  public onScrollUp(): void {
    if (this.loading_feeds == false) {
      this.loadFeed({ p: this.page_id, page: this.next_feed_page });
    }
  }
  ...
  loadFeed(params?: {}): any {
    ...
    this.loading_feeds = true;
    this.feedService.provider = this.feedService.getConfigValue('forms.getall.provider');
    this.feedService.service.getProvider(this.feedService.provider).crudconfig.route_url = 'feed/page-feed/';
    var _this = this;
    return this.feedService.service.getall(this.feedService.provider, params).subscribe(results => {
      _this.loading_feeds = false;
      if (results.isSuccess()) {
  ...
  prependFeed(feed: Feed, search_remove_first: boolean) {
    var pageOpen = this.isPageOpen as number;
    if (feed.page != null && pageOpen != null && feed.page.id == pageOpen) {
  ...
  get isPageOpen() {
    if (this.urlViewerService.PPVIEWER.type == 'page' && this.urlViewerService.PPVIEWER.page != null) {
        return this.urlViewerService.PPVIEWER.page.id;
    }
    return null;
  }
}
